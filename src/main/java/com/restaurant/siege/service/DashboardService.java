...
public class DashboardService {
  ...
  public List<SoldDish> getBestSales(LocalDateTime dateMin, LocalDateTime dateMax) {
    ...
    try (HttpClient client = HttpClient.newHttpClient()) {
      HttpRequest request = HttpRequest.newBuilder().uri(new URI("http://localhost:8081")).build();
  ...
  public ProcessingTime getProcessingTimeFor(
    ...
    try (HttpClient client = HttpClient.newHttpClient()) {
      HttpRequest request = HttpRequest.newBuilder().uri(new URI("http://localhost:8081/dishOrders?page=1&pageSize=10")).build();
      HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
